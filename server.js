...
Return ONLY a JSON array of 7 question strings, no markdown, no explanation.` }],
    ...
    const platforms = [
      ...
      { name: "Gemini",     step: 4, fn: async q => { const m = genAI.getGenerativeModel({model:"gemini-1.5-flash"}); const r = await withTimeout(m.generateContent(q)); return r.response.text(); } },
